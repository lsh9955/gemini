...
### Control NET 설정
CONTROLNET_DIR = os.path.join(os.getcwd(), 'models', 'ControlNet')
CONTROLNET_FILE = os.path.join(CONTROLNET_DIR, 'controlnetPreTrained_depthV10.safetensors')


def check_and_download_controlnet_file():
    if not os.path.isdir(CONTROLNET_DIR):
        os.makedirs(CONTROLNET_DIR)

    if not os.path.isfile(CONTROLNET_FILE):
        url = "https://civitai.com/api/download/models/11000"
        r = requests.get(url, allow_redirects=True)
        with open(CONTROLNET_FILE, 'wb') as f:
            f.write(r.content)
        print(f"ControlNet pretrained file downloaded to {CONTROLNET_FILE}.")
    else:
        print(f"ControlNet pretrained file already exists at {CONTROLNET_FILE}.")
...
def prepare_environment():
    check_and_download_controlnet_file()
